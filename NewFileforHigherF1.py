...
def extract_model_words(cleaned_title):
    ...
    # model_words = [word for word in model_words if model_id not in word]
    return model_words
...
#This method performs Minhashing on the binary matrix to create the signature matrix, where each row of the signature matrix corresponds to a permutation, and each column corresponds to a product.
...
def obtain_performance_metrics(candidate_pairs, high_similarity_pairs, dataframe):
    ...
    if n_true_positives == 0 and n_false_positives == 0:
        precision = 0
    else:
        precision = n_true_positives / (n_false_positives + n_true_positives)

    if n_true_positives == 0 and n_false_negatives == 0:
        recall = 0
    else:
    ...
    pairQuality = n_true_positives / n_comparisons
    pairCompleteness = n_true_positives / n_true_duplicates
...
def bootstrapping(df):
    ...
    for bootstrap in range(n_bootstraps):
        ...
        train_indices = np.random.choice(range(num_tvs), size=int(0.63 * num_tvs), replace=True)  ###0.63 kan weg
        ...
        for b_candidate, r_candidate in zip(possible_b_values, possible_r_values):
            ...
            performance_metrics = obtain_performance_metrics(candidate_pairs_train, high_similarity_pairs_train,
                                                             train_df)
